...
5. **CHATGPT_HANDOFF.md 갱신 필수**: 실제 코드/설정 변경이 있었던 작업을 마칠
   때마다 `CHATGPT_HANDOFF.md`를 갱신한다. 문서는 두 부분으로 구성한다:
   - **본문(현재 작업 스냅샷)**: 다음 5개 항목으로 최신 상태로 덮어쓴다(이 부분은
     과거 이력을 누적하지 않고 최신 스냅샷만 유지) — 현재 작업 / 변경 내용 /
     테스트 결과 / 발견된 문제 / 다음 결정사항.
   - **맨 아래 "최근 완료 작업 로그"**: 본문을 덮어쓰기 직전에, 그 시점의
     "현재 작업" 제목 한 줄 + 날짜 + 상태(MASTER PUSHED/PRODUCTION DEPLOYED 등)를
     이 로그 맨 위에 추가하고 **최근 5개만 유지**(6번째부터는 삭제 — git log에는
     전부 남으므로 로그 자체가 무제한 역사일 필요는 없음). 목적: 두 PC를 번갈아
     쓰는 동안 세션이 여러 번 연달아 끝나 본문 스냅샷이 반복해서 덮어써져도,
     "그 사이에 완료된 다른 작업이 있었다"는 사실 자체는 최소 몇 세션치 남아있게
     해서 다음 세션이 완전히 못 찾고 넘어가는 일을 줄인다(2026-09-23 실제로
     회사 PC에서 완료한 SSR 작업 요약이 그 뒤 두 번의 작업으로 연달아 덮어써져
     다음 세션이 못 찾은 사고 이후 반영).
