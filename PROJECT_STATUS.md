...
- 로컬 master 동기화 기준: `origin/master` 커밋 `9e9a422`(이 문서를 갱신하는 커밋 직전 기준. 이후 회사 PC에서 JobDetail 재구성/스크롤버그 수정/지도 신뢰성 개선 등 22커밋 추가 병합됨 — `master` 로그로 항상 최신 확인할 것)
...
- Home 하단 "로그인 유도 카드"를 "지원 활동 관리" 카드로 교체 (8/29): 비로그인 시 "이력서 등록"/"지원현황 보기" CTA, 로그인 시 `applications`/`message_threads`/`interviews`/CV(localStorage) 실제 카운트를 `CV·지원·메시지·면접` 한 줄로 표시. `/ho-so` 진입 시 CV/지원현황 탭으로 바로 열리도록 `Profile.tsx`에 `openApplicationsTab` 네비게이션 state 추가. 박스 크기는 왼쪽 광고 박스(현재 임시 콘텐츠, Google 광고 승인 대기용)와 데스크톱/태블릿 124px로 동일, 모바일도 오차 없이 확인
