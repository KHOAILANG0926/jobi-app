...
**"Gần tôi" 검색창을 독립 사이드바로 분리 + 검색 히스토리 추가
(2026-09-20, 네 번째 라운드) + 별도 전용 페이지(`/ban-do`) 존재 발견**:
상시 뷰(검색+지도+리스트)를 배포한 뒤 사용자가 다시 카카오맵 캡처를
보여주며 "구성이 달라 — 검색창이 독립된 사이드바로 분리 안됨" 지적 →
컨트롤(GPS/반경/검색창)이 지도 위 가로줄에 있던 걸 왼쪽 사이드바 컬럼
안으로 완전히 옮김. 이어서 "카카오맵은 상세 가게가 나오는데 우리꺼는
안나와"(POI 라벨 밀도) 지적 → Geoapify 타일 스타일 4종(osm-carto/
osm-bright/klokantech-basic/osm-liberty)을 같은 호치민 좌표로 직접
비교한 결과 **현재 쓰는 osm-carto가 오히려 라벨이 가장 많았음** —
스타일 문제가 아니라 베트남 OpenStreetMap 데이터 자체가 부족한 것으로
결론, 스타일 변경 안 함. 이어서 "그래도 구성은 만드는게 좋을거같은데"
(캡처의 검색 히스토리 섹션 재확인) → 주소 검색 히스토리(localStorage)
추가.
- **중요 발견**: 이 작업 도중 사용자가 "별도 페이지로 만들어줘"라고
  요청해서 새 페이지를 만들려다가, **이미 `/ban-do`라는 사실상 동일한
  전용 페이지([MapView.tsx](src/components/MapView.tsx))가 존재**한다는
  걸 발견함 — GPS·반경 필터·지도 핀·거리순 리스트까지 오늘 새로 만든 것과
  거의 같은 기능이 이미 있었으나, **헤더 메뉴 어디에도 연결 안 되어
  있어 사용자도 "저거 처음 보는데"라고 할 정도로 완전히 방치돼 있었음**.
  이 페이지는 지도 타일도 예전 OSM 직접 호출 방식(Geoapify 전환 이전
  레거시)이라 그대로 쓰기엔 불안정할 수 있음. **아직 결정 안 됨**: 이
  기존 `/ban-do` 페이지를 오늘 만든 개선사항으로 업데이트해서 "별도
  페이지" 요청에 쓸지, 아니면 오늘 Home.tsx에 만든 상시 뷰를 그대로
  유지할지 — 다음 세션에서 먼저 확인 필요(아래 "다음 결정사항" 참고).
- [geoapify.ts](src/lib/geoapify.ts): `getAddressSearchHistory`/
  `addAddressSearchHistory`/`clearAddressSearchHistory` 신규 — 최근
  검색어 최대 5개를 `localStorage`에만 저장(서버 전송 없음), storage.ts의
  기존 try/catch 패턴 재사용.
- [Home.tsx](src/pages/Home.tsx): `.near-me-view__split`을 "사이드바
  (검색창+히스토리+GPS+반경+요약문구+리스트 전부)" + "지도"의 완전히
  분리된 2컬럼으로 재구성 — 데스크톱 380px 고정 사이드바(세로 스크롤)
  + 가변폭 지도, 모바일은 세로 스택(사이드바 먼저, 지도 아래). `handleAddressSearch`가
  `queryOverride` 인자를 받도록 수정해 히스토리 항목 클릭 시 바로
  재검색되게 함. `renderAddressSearch()`에 "Lịch sử tìm kiếm" 섹션
  추가(검색 결과 없을 때만 표시, "Xóa hết"로 전체 삭제 가능).
- 새 CSS `.near-me-view__sidebar`/`.near-me-address-history`
  ([index.css](src/index.css)) — 기존 `.near-me-view__list`는
  `.near-me-view__sidebar`로 이름 변경(역할이 리스트 전용에서 사이드바
  전체로 확장됐으므로).
- `npx tsc --noEmit` 클린, `npm run build` 성공, `npm test` 6/6 파일
  통과. 로컬 dev 서버(1280px/375px)로 JS 직접 검증 — 데스크톱
  `grid-template-columns: "380px 825px"`이고 검색창/GPS버튼이
  `.near-me-view__sidebar` 안에 있고 `.near-me-view__map` 안엔 없음을
  확인(분리 성공), 모바일은 단일 컬럼(`356px`). 히스토리 항목 클릭 →
  입력창에 값 채워지고 검색 실행되는 것까지 확인(로컬은 키가 없어
  "Không tìm thấy địa chỉ" 응답이지만 메커니즘 자체는 정상 동작 확인).
  콘솔에 새 에러 없음. **Production 배포 후 재확인 필요**(다음 최우선
  확인 항목).

**"Gần tôi"를 카카오맵 스타일 상시 검색+지도+리스트 뷰로 전면 재구성
(2026-09-20, 같은 날 세 번째 라운드)**: 지도+핀 1차 버전을 배포한 뒤에도
사용자가 카카오맵 캡처를 다시 보여주며 "이런식으로 가자고 한거
아니었어?"로 재확인 — 1차 버전은 검색 **전**엔 여전히 작은 안내 카드만
보이고 지도는 검색 **성공 후에만** 나타나는 구조였는데(사용자가 "똑같은데"
라고 느낀 이유), 원래 합의한 건 카카오맵처럼 **검색창+지도+리스트가
처음부터 한 화면에 항상 같이 있는** 구조였다는 걸 재확인 → "ok" 받고
즉시 전면 재구성.
- [jobCoords.ts](src/lib/jobCoords.ts): `VIETNAM_CENTER`(베트남 지리적
  중심 좌표)를 `export`로 변경 — 위치 미확보 상태의 기본 지도 중심으로
  재사용(중복 상수 선언 방지).
- [Home.tsx](src/pages/Home.tsx): `nearMe` 분기를 완전히 새로 씀 — 이제
  `nearMe`가 true가 되는 순간(위치 확보 여부와 무관하게) `.near-me-view`
  섹션이 항상 렌더링되고, 그 안에:
  - **컨트롤 바**(`.near-me-view__controls`): GPS 버튼(위치 있으면 "Cập
    nhật vị trí"로 라벨 전환) + 반경 칩(위치 확보 후에만) + 주소 검색창
    (`renderAddressSearch()` 재사용) — 전부 한 줄에 항상 같이 있음. 예전
    엔 이 컨트롤들이 페이지 여러 군데(퀵필터 줄의 `near-me-controls`,
    결과 섹션의 작은 카드)에 흩어져 있었는데 하나로 합침 — 기존
    `near-me-controls` 블록(반경 칩+새로고침)은 여기로 흡수되면서 제거.
  - **지도**(`.near-me-view__map`): 위치 없으면 `VIETNAM_CENTER`로 줌 5,
    있으면 실제 좌표로 줌 13 — **섹션 자체가 나타났다 사라지지 않고 지도
    내용만 갱신**됨(핀 소스는 이전 라운드에서 고친 `resolveDistanceSearchPoint`
    그대로 유지).
  - **리스트**(`.near-me-view__list`): 위치 없으면 안내 문구, 있으면
    기존 공고 카드 그리드(그대로) 또는 0건 안내.
  - 데스크톱(≥900px)은 왼쪽 리스트(380px 고정, 세로 스크롤)+오른쪽 큰
    지도(가변폭) 2단 배치(카카오맵과 동일 구조), 모바일은 지도 위·리스트
    아래로 세로 스택.
  - "Tìm việc theo khu vực thay vào đó"(지역 패널로 스크롤 이동) 버튼과
    그 핸들러(`scrollToRegionPanel`)는 완전히 제거 — 주소 검색창이 이미
    같은 역할을 대체하고, 예전 라운드에서 "이 버튼과 검색창이 역할
    겹친다"고 정리했던 결정을 이번에 실제로 반영함.
  - **`!nearMe`(일반 브라우징) 경로는 손대지 않음** — "Tất cả kết quả"
    제목+필터 결과 그리드는 기존 그대로, near-me 전용 UI(지도/컨트롤)가
    거기 섞여 들어가지 않게 분기를 완전히 분리해서 작성.
- 새 CSS `.near-me-view`/`.near-me-view__controls`/`.near-me-view__split`/
  `.near-me-view__map`/`.near-me-view__list`/`.near-me-view__radii`
  ([index.css](src/index.css)) — 900px 기준 반응형 그리드(2단↔세로스택).
- `npx tsc --noEmit` 클린, `npm run build` 성공, `npm test` 6/6 파일
  통과. 로컬 dev 서버(1280px 데스크톱/375px 모바일)로 JS 직접 검증
  (screenshot 도구가 계속 실패해서 `document.querySelector`+
  `getComputedStyle`로 우회) — 데스크톱 `grid-template-columns:
  "380px 824.8px"`(2단 정상), 모바일 단일 컬럼(`355.8px`) 확인. 위치
  미확보 상태에서도 지도가 줌 5로 정상 렌더(베트남 전체 시작 화면) 확인.
  콘솔에 새 에러 없음. **로컬엔 Geoapify 키가 없어 실제 검색 후 지도가
  줌 13로 전환되는 것까지는 로컬에서 확인 못 함 — Production 배포 후
  전체 흐름(검색 전 베트남 시작 화면 → 검색 후 확대+핀) 재확인 필요**
  (다음 최우선 확인 항목).
...
18. **기존 `/ban-do`(MapView.tsx) 전용 지도 페이지를 어떻게 할지 —
    미정, 다음 세션 최우선 확인**(2026-09-20, 위 "검색창을 독립 사이드바로
    분리" 새 절 참고). 사용자가 "별도 페이지로 만들어줘"라고 해서 확인해
    보니, 오늘 Home.tsx에 새로 만든 것과 거의 같은 기능(GPS·반경 필터·
    지도 핀·거리순 리스트)의 전용 페이지가 **이미 존재**했음 — 다만
    헤더 메뉴 어디서도 연결이 안 돼 있어 사용자도 실사이트에서 처음
    봤다고 할 정도로 방치돼 있었고, 지도 타일도 Geoapify 전환 이전의
    레거시 OSM 직접 호출 방식이라 불안정할 수 있음. 다음 두 방향 중
    미정:
    - A) `/ban-do`를 오늘 개선사항(Geoapify 타일, 주소 검색, 히스토리,
      안전한 핀 클릭 링크)으로 업데이트하고 헤더 "Gần tôi"를 여기로
      연결, Home.tsx의 상시 뷰는 걷어냄(중복 제거).
    - B) Home.tsx의 상시 뷰를 그대로 유지하고 `/ban-do`는 계속 미연결
      상태로 두거나 삭제.
    사용자가 아직 명확히 고르지 않음 — 다음에 먼저 확인할 것.
