...
- `npx tsc --noEmit` 클린, `npm run build` 성공, `npm test` 6/6 파일 통과.
  ...
  스크린샷 확인 — 사기방지 박스 정상 렌더, 기존 레이아웃 회귀 없음. **단,
  Production에 현재 employer_id가 채워진 active 공고가 0건**(직전 세션에서
  테스트 등록 후 삭제함)이라 "Đã đăng N tin" 줄 자체의 실측 확인은 못 함 —
  로직은 기존에 검증된 공개 목록 쿼리 패턴(JobsContext.tsx)과 동일한
  필터라 구조적으로는 신뢰 가능, 다음에 실제 기업 계정이 공고를 올리면
  화면에 반영되는지 한 번 확인 필요.
- 2단계(기업인증/근로계약서약속/4대보험 배지 — 실제 검증 플로우 설계 필요)는
  이번 라운드에 포함 안 함, 아래 "다음 결정사항" 참고. 헤더 메가메뉴 탐색축
  세분화(지역별/업직종별/대상별/기간별/근무조건별 전용 진입점)도 범위 밖,
  별도 라운드로 보류.
...
12. ~~`backup-home-2026-09-18-workperiod-panel` 브랜치 재적용 여부~~ —
    ...
    원격에 영향 없음).
13. **알바몬 구조 벤치마킹 — 2단계/나머지 항목**(2026-09-20, 위 새 절
    참고): 신뢰 정보 1단계(가입 데이터 기반 카드+사기방지 안내)만 이번에
    진행. 남은 항목:
    - **기업인증/근로계약서작성약속/4대보험 배지** — 실제 검증 절차(사업자
      등록증 확인 등 관리자 승인 플로우) 설계가 먼저 필요, 새 DB 컬럼 +
      운영 프로세스 필요해 STRICT 대상. 착수 여부/방식 결정 필요.
    - **헤더 메가메뉴 탐색축 세분화** — 알바몬은 지역별/업직종별/대상별/
      기간별/근무조건별로 각각 전용 진입점을 두는데 우리는 필터로만
      존재. 베트남에 안 맞는 축(청소년/장년/장애인 대상별 등)은 제외하고
      어떤 축부터 진입점을 만들지는 아직 미정.
    - **상세페이지 탭 구조**(근무조건/상세요강/기업정보) — 알바몬은 탭으로
      나뉘어 있고 우리는 한 페이지 스크롤. 더 큰 레이아웃 변경이라 이번
      1단계에 포함하지 않음, 별도 결정 필요.
