...
{
  const jobs = [
    job({ company: 'Jollibee Việt Nam', category: 'restaurant' }),
    job({ company: 'Jollibee Việt Nam', category: 'restaurant' }),
  ]
...
{
  const jobs = [
    job({ company: 'Chuỗi Cafe ABC', category: 'cafe', title: 'Pha chế 1', description: 'hệ thống nhiều chi nhánh' }),
    job({ company: 'Chuỗi Cafe ABC', category: 'cafe', title: 'Pha chế 2', location: 'TP.HCM' }),
  ]
  const candidates = detectBrandCandidates(jobs, registeredBrands, new Set())
...
{
  const jobs = [job({ company: 'Single Shop', category: 'retail' })]
  const candidates = detectBrandCandidates(jobs, registeredBrands, new Set())
...
{
  const jobs = [
    job({ company: 'Samsung Electronics Việt Nam', category: 'factory' }),
    job({ company: 'Samsung Electronics Việt Nam', category: 'factory' }),
  ]
  const candidates = detectBrandCandidates(jobs, registeredBrands, new Set())
...
// 5) 대상 업종이 아니면(factory/office/cleaning/other) 여러 건이어도 후보가 아니다.
{
  const jobs = [
    job({ company: 'Văn Phòng Test', category: 'office' }),
    job({ company: 'Văn Phòng Test', category: 'office' }),
  ]
  const candidates = detectBrandCandidates(jobs, registeredBrands, new Set())
...
{
  const jobs = [
    job({ company: 'Dismissed Co', category: 'retail' }),
    job({ company: 'Dismissed Co', category: 'retail' }),
  ]
