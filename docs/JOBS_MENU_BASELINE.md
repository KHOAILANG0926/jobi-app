...
### 7-3. 실제 위치 결과 재현성 조사 + "지역별 검색" 버튼 무반응 결함 수정 (2026-09-10)

- **1) "현재 위치 사용" 결과가 항상 하노이 5km·16건으로 보인다는 제보 조사**:
  코드·환경을 직접 재조사했다 — 결론: **코드 결함 없음, 재현 안 됨**.
  - `src/pages/Home.tsx`의 `handleQuickNearMe()`는 `pos.coords.latitude/
    longitude`(브라우저가 실제로 반환한 값)만 그대로 `userCoords`에 쓴다 —
    하노이·다른 지역으로 대체하는 fallback이나 하드코딩된 좌표가 없다
    (전체 소스 검색으로 확인 — `21.0285/105.8542`는 `jobCoords.ts`의
    지역명↔중심좌표 텍스트 매칭 표와 `mockJobs.ts`의 데모 공고 좌표에만
    있고, 둘 다 `handleQuickNearMe` 경로와 무관).
  - `userCoords`는 컴포넌트 `useState`일 뿐 localStorage 등에 저장되지
    않아 새로고침/재방문 시 항상 초기화된다 — "이전 상태 잔재"가 남을
    구조 자체가 없다.
  - 이번 조사 중 사용한 브라우저 자동화 도구(mock 없이 운영 사이트에서
    `navigator.geolocation.getCurrentPosition`을 직접 호출)는 오히려
    `permission state: denied`(권한 거부)를 반환했다 — 즉 이 자동화
    환경도 임의로 하노이를 반환하는 내장 설정이 없다는 것도 함께 확인됨.
  - 이전 라운드에서 성공/실패 상태를 검증할 때 쓴 `navigator.geolocation`
    모의(mock)는 로컬 개발 서버 탭의 자바스크립트 콘솔에서만 실행한
    임시 객체 오버라이드였다 — 소스 코드에 커밋된 적이 없고, 다른
    브라우저·세션·이 문서를 보는 실제 사용자에게 전달될 수 있는 경로가
    아니다.
  - **결론**: "하노이 5km 16건"이 실제 사용자에게 반복해서 보였다면, 그
    사용자의 브라우저/OS가 GPS·Wi-Fi 위치를 못 구해 IP 기반 위치로
    대체(fallback)했을 가능성이 가장 유력하다 — 이는 브라우저·OS가 결정하는
    값이고 우리 코드가 대체 좌표를 주입하는 게 아니다. 다만 이 설명이
    맞는지는 그 사용자의 실제 기기에서만 확인 가능하며, 이번 조사에서
    임의로 "사용자가 하노이에 있다"고 단정하지 않는다 — 코드 수정 없음.
- **2) "Tìm việc theo khu vực thay vào đó" 버튼 무반응 결함 — 실제 확인·수정**:
  이전 라운드는 `scrollIntoView`가 호출되는지만 확인하고 통과 처리했는데,
  이번에 버튼을 실제로 클릭해보니 **`scrollIntoView`가 전혀 호출되지
  않았다**(호출 기록 0건). 원인: `scrollToRegionPanel()`이
  `window.requestAnimationFrame()`으로 감싸져 있었는데, 탭이 실제로
  화면을 그리는 중이 아니면(자동화 도구의 "hidden" 상태에서 실측 재현)
  rAF 콜백 자체가 실행되지 않아 클릭이 아무 효과도 못 냈다. 수정: `ref`는
  클릭 시점에 이미 마운트돼 있어 다음 프레임을 기다릴 이유가 없으므로
  `regionPanelRef.current?.scrollIntoView(...)`를 rAF 없이 클릭 즉시
  동기 호출하도록 변경 — 이 연결 하나만 수정(다른 스크롤 헬퍼는 손대지
  않음). 수정 후 재확인: `scrollIntoView`가 정확히 1회, 올바른 대상
  (`.home-region-panel`)에 호출됨을 확인. 단, 이 세션 내내 Browser 패널이
  "hidden"으로 보고돼 실제 화면 스크롤 완료의 육안 확인은 이번에도 못
  했다 — 호출이 안 되던 결함은 확실히 고쳤지만, 완료된 스크롤의 시각적
  확인은 남은 사항으로 별도 표시한다.
